...
function setUnvalidatedAtomValue(state, key, newValue) {
  ...
  node === null || node === void 0
    ? void 0
    : (_node$invalidate = node.invalidate) === null || _node$invalidate === void 0
    ? void 0
...
function getDownstreamNodes(store, state, keys) {
  ...
  for (let key = visitingNodes.pop(); key; key = visitingNodes.pop()) {
    ...
    const subscribedNodes =
      (_graph$nodeToNodeSubs = graph.nodeToNodeSubscriptions.get(key)) !== null && _graph$nodeToNodeSubs !== void 0
...
const { DefaultValue: DefaultValue$1, RecoilValueNotReady: RecoilValueNotReady$1 } = Recoil_Node;
...
function valueFromValueOrUpdater(store, { key }, valueOrUpdater) {
  if (typeof valueOrUpdater === 'function') {
    ...
    const state =
      (_storeState$nextTree = storeState.nextTree) !== null && _storeState$nextTree !== void 0
...
function markRecoilValueModified(store, { key }) {
  Recoil_Tracing.trace('mark RecoilValue modified', key, () =>
    store.replaceState(
      Recoil_Tracing.wrap((state) => ({ ...state, dirtyAtoms: Recoil_unionSets(state.dirtyAtoms, new Set([key])) })),
    ),
  );
}
...
const { getDownstreamNodes: getDownstreamNodes$1, peekNodeLoadable: peekNodeLoadable$1 } = Recoil_FunctionalCore;

const { graph: graph$2 } = Recoil_Graph;

const { DEFAULT_VALUE: DEFAULT_VALUE$1, recoilValues: recoilValues$1 } = Recoil_Node;
...
const { getNextTreeStateVersion: getNextTreeStateVersion$1, makeEmptyStoreState: makeEmptyStoreState$1 } = Recoil_State; // Opaque at this surface because it's part of the public API from here.
...
class Snapshot {
  constructor(storeState) {
    ...
    _defineProperty(this, 'getNodes_UNSTABLE', (opt) => {
      ...
      return (opt === null || opt === void 0 ? void 0 : opt.isInitialized) == null
        ? recoilValues$1.values()
        : opt.isInitialized === true
        ? recoilValuesForKeys(
            Recoil_concatIterables([this._store.getState().knownAtoms, this._store.getState().knownSelectors]),
          )
    ...
    _defineProperty(this, 'getInfo_UNSTABLE', (recoilValue) => {
      ...
      return {
        ...
        deps: recoilValuesForKeys(
          (_graph$nodeDeps$get = graph.nodeDeps.get(key)) !== null && _graph$nodeDeps$get !== void 0
            ? _graph$nodeDeps$get
            : [],
        ),
...
const { graph: graph$3, saveDependencyMapToStore: saveDependencyMapToStore$2 } = Recoil_Graph;
...
const { getNextTreeStateVersion: getNextTreeStateVersion$2, makeEmptyStoreState: makeEmptyStoreState$2 } = Recoil_State;

const { mapByDeletingMultipleFromMap: mapByDeletingMultipleFromMap$2 } = Recoil_CopyOnWrite;
...
function sendEndOfBatchNotifications(store) {
  ...
  if (dirtyAtoms.size) {
    ...
    storeState.suspendedComponentResolvers.forEach((cb) =>
      Recoil_Tracing.trace('value became available, waking components', nodeNames, cb),
    );
...
function RecoilRoot({
  ...
  const createMutableSource =
    (_createMutableSource = react.createMutableSource) !== null && _createMutableSource !== void 0
    ...
    storeProp !== null && storeProp !== void 0
      ...
      : {
          ...
          mutableSource: createMutableSource
            ? createMutableSource(storeState, () => storeState.current.currentTree.version)
            : null,
        };
...
const { useCallback, useEffect: useEffect$1, useMemo, useRef: useRef$1, useState: useState$1 } = react;

const { DEFAULT_VALUE: DEFAULT_VALUE$2, getNode: getNode$2, nodes: nodes$1 } = Recoil_Node;
...
const { Snapshot: Snapshot$1, cloneSnapshot: cloneSnapshot$1 } = Recoil_Snapshot$1;
...
function useRecoilValueLoadable_MUTABLESOURCE(recoilValue) {
  ...
  const treeState = useMutableSource(storeRef.current.mutableSource, getTreeState, subscribe);
...
const useMutableSource =
  (_useMutableSource = react.useMutableSource) !== null && _useMutableSource !== void 0
    ? _useMutableSource
            ...
            persistence_UNSTABLE: {
              type:
                (_node$persistence_UNS =
                  (_node$persistence_UNS2 = node.persistence_UNSTABLE) === null || _node$persistence_UNS2 === void 0
                    ? void 0
                    : _node$persistence_UNS2.type) !== null && _node$persistence_UNS !== void 0
                  ? _node$persistence_UNS
                  : 'none',
              backButton:
                (_node$persistence_UNS3 =
                  (_node$persistence_UNS4 = node.persistence_UNSTABLE) === null || _node$persistence_UNS4 === void 0
                    ? void 0
                    : _node$persistence_UNS4.backButton) !== null && _node$persistence_UNS3 !== void 0
                  ? _node$persistence_UNS3
                  : false,
            },
          };
      ...
      reactDom.unstable_batchedUpdates(() => {
        ...
        for (const keys of [prev.atomValues.keys(), next.atomValues.keys()]) {
          for (const key of keys) {
            ...
            if (
              ((_prev$atomValues$get = prev.atomValues.get(key)) === null || _prev$atomValues$get === void 0
                ? void 0
                : _prev$atomValues$get.contents) !==
                ((_next$atomValues$get = next.atomValues.get(key)) === null || _next$atomValues$get === void 0
                  ? void 0
        ...
        keysToUpdate.forEach((key) => {
          setRecoilValueLoadable$1(
            ...
            next.atomValues.has(key) ? Recoil_nullthrows(next.atomValues.get(key)) : DEFAULT_VALUE$2,
          );
        });
  ...
  return (values, transactionMetadata = {}) => {
    reactDom.unstable_batchedUpdates(() => {
      ...
      values.forEach((value, key) =>
        setUnvalidatedRecoilValue$1(storeRef.current, new AbstractRecoilValue$2(key), value),
      );
    });
  };
}
    ...
    (...args) => {
      ...
      !!(ret instanceof Sentinel)
        ? process.env.NODE_ENV !== 'production'
          ? Recoil_invariant(false, 'unstable_batchedUpdates should return immediately')
...
const { useStoreRef: useStoreRef$2 } = Recoil_RecoilRoot_react;

function useRecoilStore() {
  return useStoreRef$2().current;
}

var Recoil_useRecoilStore = useRecoilStore;
...
function isNode(object) {
  ...
  const doc =
    object != null
      ? (_ownerDocument = object.ownerDocument) !== null && _ownerDocument !== void 0
        ? _ownerDocument
        : object
      : document;
  const defaultView =
    (_doc$defaultView = doc.defaultView) !== null && _doc$defaultView !== void 0 ? _doc$defaultView : window;
  return !!(
    object != null &&
...
function selector(options) {
  ...
  function getFromCacheOrEvaluate(store, state) {
    ...
    const currentDeps =
      (_store$getGraph$nodeD = store.getGraph(state.version).nodeDeps.get(key)) !== null &&
      _store$getGraph$nodeD !== void 0
  ...
  function myPeek(store, state) {
    ...
    const currentDeps =
      (_store$getGraph$nodeD2 = store.getGraph(state.version).nodeDeps.get(key)) !== null &&
      _store$getGraph$nodeD2 !== void 0
...
const { DEFAULT_VALUE: DEFAULT_VALUE$4, DefaultValue: DefaultValue$2, registerNode: registerNode$2 } = Recoil_Node;
        ...
        : {
            ...
            validator: (storedValue) =>
              storedValue instanceof DefaultValue$2
                ? storedValue
                : Recoil_nullthrows(options.persistence_UNSTABLE).validator(storedValue, DEFAULT_VALUE$4),
          },
    // TODO Hack for now.
    // flowlint-next-line unclear-type: off
...
function selectorFamily(options) {
  ...
  let selectorCache =
    (_options$cacheImpleme =
      (_options$cacheImpleme2 = options.cacheImplementationForParams_UNSTABLE) === null ||
      _options$cacheImpleme2 === void 0
        ? void 0
        : _options$cacheImpleme2.call(options)) !== null && _options$cacheImpleme !== void 0
      ? _options$cacheImpleme
  ...
  return (params) => {
    ...
    const myCacheImplementation =
      (_options$cacheImpleme3 = options.cacheImplementation_UNSTABLE) === null || _options$cacheImpleme3 === void 0
        ? void 0
...
const { DEFAULT_VALUE: DEFAULT_VALUE$5, DefaultValue: DefaultValue$3 } = Recoil_Node;
/*
...
function atomFamily(options) {
  ...
  return (params) => {
    ...
    const newAtom = Recoil_atom({
      ...options,
      key: `${options.key}__${
        (_stableStringify = Recoil_stableStringify(params)) !== null && _stableStringify !== void 0
          ? _stableStringify
          : 'void'
      }`,
  ...
  get: (dependencies) => ({ get }) => {
    ...
    if (Recoil_gkx_1('recoil_async_selector_refactor')) {
      ...
      return Promise.all(exceptions).then((results) =>
        wrapResults(dependencies, results.map(getValueFromLoadablePromiseResult)),
      );
    } else {
...
var Recoil_index = {
  ...
  useRecoilStore_UNSTABLE: Recoil_useRecoilStore,
  // Concurrency Helpers
  noWait: noWait$1,
  waitForNone: waitForNone$1,
  waitForAny: waitForAny$1,
...
var Recoil_index_22 = Recoil_index.useRecoilStore_UNSTABLE;
...
exports.useRecoilStore_UNSTABLE = Recoil_index_22;
